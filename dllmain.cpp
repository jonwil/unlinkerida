...
inline entry_chooser_t::entry_chooser_t()
    : chooser_t(CH_KEEP | CH_NOIDB | CH_CAN_DEL,
...
{
    if (std::find_if(entries.begin(), entries.end(), [e](unlink_entry e2) {return e.ea == e2.ea && e.module_index == e2.module_index; }) == entries.end())
    {
        entries.push_back(e);
...
void export_unlinked_module(qstring name, qvector<unlink_entry>& vector)
{
    ...
    if (path)
    {
        ...
        *((unsigned int*)DebugSSectionPos) = DebugSSymbolsSize;
        ...
        int FeatureEnum = Unknown1 | Unknown2 | Unknown3 | SafeSEH | Unknown4;
